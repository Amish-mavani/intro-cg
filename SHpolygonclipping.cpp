...
int main() {
	...
	initwindow(screenWidth, screenHeight, "Curve");
	...
	for (int i = 0; i < numLines; ++i) {
		cout << "Enter coordinates for point P" << (i + 1) << " (x1 y1): ";
		cin >> x1[i] >> y1[i] ;
	...
	for (int i = 0; i < numLines; ++i) {		
        if (i < numLines-1){
		setcolor(LIGHTRED);
		line(x1[i] + centerX, centerY - y1[i], x1[i+1] + centerX, centerY - y1[i+1]);}
        else{
		setcolor(LIGHTRED);
		line(x1[i] + centerX, centerY - y1[i], x1[0] + centerX, centerY - y1[0]);};		
	}
	delay(2000);
	for (int i = 0; i < numLines; ++i) {
		if (i < numLines-1){
		...
		if (i < numLines-1){
		cohenSutherland(x1[i] + centerX, centerY - y1[i], x1[i+1] + centerX, centerY - y1[i+1], xmin + centerX, centerY - ymax, xmax + centerX, centerY - ymin);}
        else{
		setcolor(WHITE);
		cohenSutherland(x1[i] + centerX, centerY - y1[i], x1[0] + centerX, centerY - y1[0], xmin + centerX, centerY - ymax, xmax + centerX, centerY - ymin);}
